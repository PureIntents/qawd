...
# Verifica si el archivo existe antes de ejecutarlo
if (Test-Path -Path $filePath) {
    Write-Host "El archivo existe en la ruta especificada."
    # Ejecuta el archivo
    & $filePath
    Write-Host "El archivo se ha ejecutado correctamente."
    # Espera a que el archivo se cierre por el sistema operativo
    Start-Sleep -Seconds 5
    Write-Host "Se ha esperado 5 segundos para que el archivo se cierre."
    # Elimina el archivo
    try {
        Remove-Item -Path $filePath -Force
        Write-Host "El archivo se ha eliminado correctamente."
    } catch {
